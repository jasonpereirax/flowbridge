...
SEMANTIC CONTEXT:
- Purpose: The "Selected Clients" section of a public marketing/portfolio landing page for
  a digital designer. Static, content-driven — it lists the designer's selected client
  projects. No authentication, no data mutation.
- Route: this section is part of the home page at / (app/page.tsx). Render it as a Server
  Component — do NOT add "use client" (there is no interactivity beyond links).
- Repeated pattern: the section renders a LIST of 5 IDENTICAL rows
  (avatar dot + client name + category + year + "See more" link). Factor ONE reusable
  <ProjectRow> component and map over a typed data array — do not hand-write 5
  near-identical blocks.
- Shared primitive: the section header (label + plus icon) is a recurring pattern across
  the page (also used by "Objective process" and "Testimonials"). Factor a <SectionHeader title />.
- Data: there is no API. Define a typed `projects` array (name, category, year, href) and
  map over it. "See more" is a per-project link (href).
- State / notes: no loading / error / empty states (static content). Use Tailwind; avoid
  duplicating long utility class strings across the rows — keep them in the single
  <ProjectRow>. Strict TS: type the data and the component props explicitly.

Use this context to place files correctly, extract the repeated pattern into a reusable
component + typed data, and wire the links accurately.
