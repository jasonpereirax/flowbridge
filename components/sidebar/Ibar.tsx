...
interface IbarButton { section: EbarSection; icon: React.ReactNode; label: string }
...
export function Ibar() {
  ...
  return (
    <div className="flex flex-col items-center gap-[4px] w-[44px] flex-shrink-0 border-r border-border bg-surface pt-[8px] z-30">
      {BUTTONS.map(btn => {
        ...
        return (
          ...
          >
            {btn.icon}
            {/* tooltip */}
            <span className="absolute left-[calc(100%+9px)] top-1/2 -translate-y-1/2 bg-text-1 text-white text-[11px] font-medium px-2 py-[3px] rounded-[6px] whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-[100]">
              {btn.label}
            </span>
          </button>
        )
