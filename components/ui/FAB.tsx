...
export function FAB() {
  ...
  useEffect(() => {
    if (!fabOpen) return
    const onDown = (e: MouseEvent) => { if (!wrapRef.current?.contains(e.target as Node)) store.closeFab() }
  ...
  return (
    <div ref={wrapRef} className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-[8px]">

      {/* options tray */}
      <div className={cn(
        'flex items-end gap-[10px] transition-all duration-[180ms]',
        fabOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-[6px] pointer-events-none',
      )}>
        {options.map(opt => (
          <button
            key={opt.label}
          ...
          >
            <div className={cn(
              'w-[40px] h-[40px] rounded-[12px] flex items-center justify-center shadow-md transition-transform duration-[140ms] group-hover:scale-[1.08] group-active:scale-[.96]',
              opt.colorClass,
            )}>
              {opt.icon}
            </div>
      ...
      <button
        ...
        className={cn(
          'w-[42px] h-[42px] rounded-[13px] bg-text-1 text-white flex items-center justify-center outline-none select-none',
          'text-[22px] font-light leading-none',
        ...
        style={{ transition: 'transform 170ms cubic-bezier(.16,1,.3,1), box-shadow 170ms ease' }}
      >
        +
      </button>
    </div>
