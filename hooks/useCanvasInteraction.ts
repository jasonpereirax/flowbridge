...
export interface ConnDragState {
  ...
  x2: number
  y2: number
  /** If this is a reconnect drag, the original connId to delete on drop */
...
export function useCanvasInteraction(
  ...
  const onPointerDown = useCallback((e: PointerEvent) => {
    ...
    // Connector handle on DS node — start new conn drag
  ...
  /** Called from ConnectorLayer when user grabs a reconnect handle */
  const startReconnect = useCallback((
    ...
    const state = useStore.getState()
    const conn  = state.canvas()?.conns.find(c => c.id === connId)
    if (!conn) return
    ...
    // When dragging the TO endpoint, anchor is the FROM side and vice versa
    connDrag.current = {
  ...
  const onPointerUp = useCallback((e: PointerEvent) => {
    if (connDrag.current) {
      ...
      const reconnectConnId = connDrag.current.reconnectConnId
